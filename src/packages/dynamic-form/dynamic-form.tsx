  ...
  const defaultValues = useMemo(() => {
    return fields.reduce((acc, field) => {
      if (field.defaultValue !== undefined) {
        acc[field.id as keyof DefaultValues<TFieldValues>] =
  ...
  const renderInput = useCallback(
    (
      ...
      const a11yProps = {
        ...
        "aria-describedby":
          [field.helperText ? descriptionId : null, hasError ? errorId : null]
            .filter(Boolean)
            .join(" ") || undefined,
      };
      ...
      if (field.type === "select" && SelectField) {
        return (
          <SelectField {...rhfProps} {...a11yProps} options={field.options} />
        );
      }
    ...
    >
      <div className="grid grid-cols-12 gap-x-4 gap-y-6">
        {fieldItems.map((field) => {
          ...
          const safeColSpan =
            colSpanClasses[field.colSpan ?? 12] || "col-span-12";
