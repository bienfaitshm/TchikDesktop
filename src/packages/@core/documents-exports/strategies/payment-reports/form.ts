...
interface DocumentExportLayout {
  ...
  readonly examenDayNumberColSpan: ColSpan;
}

const DEFAULT_LAYOUT: Readonly<DocumentExportLayout> = {
  fileTypeColSpan: 4,
  sessionColSpan: 5,
  examenDayNumberColSpan: 3,
} as const;
 ...
 * ```
 */
export const createSeatingPresenceExportForm = async (
  ...
  const { fileTypeColSpan, sessionColSpan, examenDayNumberColSpan } = {
    ...DEFAULT_LAYOUT,
    ...layout,
  };
  ...
  return composeFields(
    ...
    NumberDaysFieldFactory.create({
      colSpan: examenDayNumberColSpan,
      helperText: "Nombre de jours des examens",
    }),
  );
};
