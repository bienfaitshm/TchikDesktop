...
const InfoView: React.FC<
  React.PropsWithChildren<{ title?: string; value: string }>
> = ({ children, value, title = "Modifier les informations" }) => {
...
const StudentEnrollmentInfos = ({
  enrollment,
}: {
  enrollment: StudentDetails;
}) => {
  const sectionTranslation = useMemo(
    () => getSectionLabel(enrollment.ClassRoom.section),
    [enrollment],
  );
  return (
    <InfoView value="edit-enrollment-infos">
      ...
      <TextInfo
        title="Nouvel élève"
        value={enrollment.isNewStudent ? "Oui" : "Non"}
      />
    </InfoView>
  );
};
...
const InfoEdit: React.FC<
  React.PropsWithChildren<{ value: string; isLoading?: boolean }>
> = ({ children, value, isLoading }) => {
  const handleChangeFrame = useChangeAnimatedFrame();
  return (
    <FormSubmitter>
      <FormSubmitter.Wrapper>{children}</FormSubmitter.Wrapper>
...
const EditEnrollmentInfos = ({
  ...
  const classroomsOptions = useGetClassroomAsOptions(
  ...
  const onSubmit = useCallback(
    (data: EnrollmentFormData) => {
      updateMutation.mutate(
        { id: enrollment.enrolementId, data },
...
export const StudentDetailsCard = ({
  ...
  yearId,
  data,
  onRefresh: globalRefesh,
}: StudentDetailsCardProps) => {
  const { classroomId } = useParams<{ classroomId: string }>();
  const { data: enrollment, refetch } = useGetEnrollmentById(
    data.enrolementId,
    { initialData: data },
  );
      ...
      {enrollment.classroomId !== classroomId && (
        <Alert>
          ...
          <AlertDescription className="text-xs">
            Cet élève ne fait plus partie de cette classe. Il a peut-être été
            transféré.
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-xs">
                Informations sur la scolarité actuelle de l'élève.
              </CardDescription>
            </div>
