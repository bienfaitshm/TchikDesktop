...
import { SeatingSessionColumns, enhanceColumnsExpandable } from "@/renderer/components/tables/columns" 
 ...
 */
const SessionRowActions = React.memo(({ session }: { session: TSeatingSession }) => {
    ...
    return (
        <ActionContainer className="lg:grid-cols-3">
            ...
            <UpdateSeatingSessionDialog seatingSessionId={session.sessionId} initialData={initialData}>
                <ActionTileEdit />
            </UpdateSeatingSessionDialog>
...
export const SeatingPage = () => {
    ...
    const { data: sessions = [] } = useGetSeatingSessions({ schoolId, yearId })
                ...
                header={
                    <section className="container flex items-center justify-between w-full max-w-screen-2xl my-4">
                        <header className="space-y-1">
                            <h1 className="text-2xl font-bold tracking-tight">Sessions de mise en place</h1>
                        ...
                        <CreateSeatingSessionDialog defaultValues={{ schoolId, yearId }}>
                            <Button size="sm" className="rounded-full shadow-sm bg-primary hover:bg-primary/90">
