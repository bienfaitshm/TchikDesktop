...
interface UseSeatingSessionFormConfig {
  ...
  onSuccess?: (data?: SeatingAssignementFormData) => void;
}

/**
 * Hook interne pour la logique partagée des formulaires d'Option.
 */
function useSeatingSessionFormBase(config?: UseSeatingSessionFormConfig) {
  const formId = useId();
  const queryClient = useQueryClient();

  const notifyAndInvalidate = useCallback(
    (data: SeatingAssignementFormData) => {
 ...
 */
export function useCreateOptionForm(config?: UseSeatingSessionFormConfig) {
  const { formId, notifyAndInvalidate } = useSeatingSessionFormBase(config);
  const mutation = useCreateOption();

  const createOption: BaseFormProps<SeatingAssignementFormData>["onSubmit"] =
    useCallback(
      (data, helpers) => {
        mutation.mutate(
          data,
 ...
 */
export function useUpdateOptionForm(config?: UseSeatingSessionFormConfig) {
  const { formId, notifyAndInvalidate } = useSeatingSessionFormBase(config);
  const mutation = useUpdateOption();

  const updateOption: BaseFormProps<
    TQueryUpdate<SeatingAssignementFormData>
 ...
 */
export function useDeleteOptionForm(config?: UseSeatingSessionFormConfig) {
