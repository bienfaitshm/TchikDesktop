...
import {
  ...
  useFormHandleRef,
} from "@/renderer/components/form/enrollments/quick-enrolement-form";
...
export const EnrollmentDialog: React.FC<EnrollmentDialogProps> = ({
  ...
  const [prevEnrollement, setPrevEnrollement] = React.useState<
    TEnrolementAttributes | undefined
  >(undefined);
      ...
      <DialogContent className="min-w-max max-w-2xl" aria-disabled>
        <FormSubmitter>
          ...
          {prevEnrollement && (
            <div>
              <TypographySmall>
                Dernier enregistrement: {prevEnrollement.code}
              </TypographySmall>
            </div>
          )}
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button id="close-dialog-button" variant="outline">
                Annuler
              </Button>
            </DialogClose>
