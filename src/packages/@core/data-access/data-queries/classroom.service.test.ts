...
import { ClassroomService } from "@/main/db/services/classroom.service"; // Assurez-vous que l'alias est correct
import { ClassRoom, Option, StudyYear } from "@/main/db/models/model";
...
describe("ClassroomService", () => {
  ...
  describe("getClassrooms", () => {
    ...
    it("should throw a service error on DB failure", async () => {
      ...
      await expect(
        ClassroomService.getClassrooms({ schoolId: MOCK_SCHOOL_ID })
  ...
  describe("getClassroomById", () => {
    it("should return the classroom DTO with relations if found", async () => {
      ...
      // Act
      const result = await ClassroomService.getClassroomById(MOCK_CLASS_ID);
    ...
    it("should return null if ID is empty or not found", async () => {
      ...
      expect(await ClassroomService.getClassroomById("")).toBeNull();
      expect(
        await ClassroomService.getClassroomById("non-existent")
      ).toBeNull();
    });
  });
  ...
  describe("updateClassroom", () => {
    it("should update and return the updated classroom DTO", async () => {
      ...
      // Act
      const result = await ClassroomService.updateClassroom(MOCK_CLASS_ID, {
        identifier: "New",
      });
    ...
    it("should return null if classroom ID is not found", async () => {
      ...
      const result = await ClassroomService.updateClassroom("999", {
        identifier: "Test",
      });
  ...
  describe("deleteClassroom", () => {
    it("should return true if deletion was successful (row count > 0)", async () => {
      ...
      const result = await ClassroomService.deleteClassroom(MOCK_CLASS_ID);
    ...
    it("should throw service error on DB constraint failure", async () => {
      ...
      await expect(
        ClassroomService.deleteClassroom(MOCK_CLASS_ID)
