...
import type { BaseStudent } from "@/packages/@core/data-access/schema-validations";
...
export function useCreateUserForm(config?: UseUserFormConfig) {
  ...
  const createUser: BaseFormProps<BaseStudent>["onSubmit"] = useCallback(
    (data, helpers) => {
      mutation.mutate(
        data,
...
export function useUpdateUserForm(config?: UseUserFormConfig) {
  ...
  const updateUser: BaseFormProps<TQueryUpdate<BaseStudent>>["onSubmit"] =
    useCallback(
      ({ data, id }, helpers) => {
        mutation.mutate(
          { data, id },
