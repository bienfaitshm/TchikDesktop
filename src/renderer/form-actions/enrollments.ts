...
import {
  ...
  useUpdateEnrollment,
  useDeleteEnrollment,
} from "@/renderer/libs/queries/enrollements/enrollments";
 ...
 */
function useEnrollmentBase<T>(config?: UseEnrolementConfig<T>) {
  const formId = useId();
...
export function useCreateQuickEnrollmentForm(
  config?: UseEnrolementConfig<EnrollmentQuickCreate>,
) {
  const { formId, notifyAndInvalidate } = useEnrollmentBase(config);
  ...
  return {
    formId,
    onSubmit: createQuickEnrolement,
...
export function useCreateEnrollmentForm(
  config?: UseEnrolementConfig<EnrolementData>,
) {
  const { formId, notifyAndInvalidate } = useEnrollmentBase(config);
  const mutation = useCreateEnrollment();

  const createEnrolement: BaseFormProps<EnrolementData>["onSubmit"] =
    useCallback(
      (data, helpers) => {
        mutation.mutate(
          data,
...
export function useUpdateEnrollmentForm(
  ...
  const updateEnrolement: BaseFormProps<
    TQueryUpdate<EnrolementData>
 ...
 */
export function useDeleteEnrollmentForm(config?: UseEnrolementConfig<void>) {
  const queryClient = useQueryClient();
